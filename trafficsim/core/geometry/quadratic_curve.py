...
class QuadraticCurve(Segment):
    ...
    def compute_dx(self, t):
        return 2 * t * (self.end[0] - 2 * self.control[0] + self.start[0]) + 2 * (
            self.control[0] - self.start[0]
        )

    def compute_dy(self, t):
        return 2 * t * (self.end[1] - 2 * self.control[1] + self.start[1]) + 2 * (
            self.control[1] - self.start[1]
        )
